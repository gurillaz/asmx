              ...
              <v-row>
                <v-col cols="auto" class="py-0 my-0">
                  <v-card-title class="pl-0 white--text">
                    Kategoria e produktit: [{{ category.id }}]
                    {{ category.name }}
                  </v-card-title>
                </v-col>
                <v-spacer></v-spacer>
            ...
            <v-container fluid class="py-1">
              <v-row class="pr-2">
                ...
                <v-col cols="auto">
                  ...
                  <p class="py-0 my-0">
                    {{
                      moment(category_copy.created_at).format(
                        "DD-MM-YY HH:mm"
                      )
                    }}
                  </p>
...
export default {
  ...
  methods: {
    submit() {
      ...
      data.append("name", this.category_copy.name || "");


      this.$inertia.post(
        this.route + "/product/category/" + this.category.id,
        data
      );
    },
  },
  mounted() {
    this.category_copy = JSON.parse(JSON.stringify(this.category));
  },
